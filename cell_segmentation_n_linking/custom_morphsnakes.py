...
def morphological_chan_vese(
    ...
    return_evolution : bool, optional
        Whether to return the evolution of the level sets during the iteration.
    early_stop : bool, optional
        Whether to stop the iteration early if the level set does not change
        for `early_stop_iter`. Must be greater than 2.
    early_stop_iter : uint, optional
        Number of iterations to wait before stopping the iteration if the level
        set does not change.
    ...
    evolution : ndarray, optional
        Evolution of the level sets during the iteration. Only returned if
        `return_evolution` is True.
    ...
    .. [1] A Morphological Approach to Curvature-based Evolution of Curves and
           ...
           2014, DOI 10.1109/TPAMI.2013.106
    """
    # check early_stop_iter
    if early_stop_iter < 2:
        raise ValueError("early_stop_iter must be greater than 2")
    ...
    if return_evolution or early_stop:
        evolution = []
        iter_callback = store_evolution_in(evolution)
    ...
    for _ in range(num_iter):
        ...
        u[aux < 0] = 1 # type: ignore
        u[aux > 0] = 0 # type: ignore
        ...
        if early_stop and len(evolution) > early_stop_iter:  # early stop check
            if np.any([np.array_equal(evolution[-1], evolution[-i]) for i in range(2, early_stop_iter + 1)]):
                break

    if return_evolution:
        return u, np.array(evolution)
    else:
        return u


def morphological_geodesic_active_contour(
    gimage, num_iter, init_level_set="circle", smoothing=1, threshold="auto", balloon=0, iter_callback=lambda x: None
):
    ...
    for _ in range(num_iter):
        ...
        if balloon != 0:
            u[threshold_mask_balloon] = aux[threshold_mask_balloon] # type: ignore
